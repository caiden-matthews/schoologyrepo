...
exports.handler = async (event) => {
  ...
  if (event.httpMethod !== 'POST') {
    return { statusCode: 405, headers: cors, body: JSON.stringify({ ok: false, error: 'Sync request must use POST method. Please try again.' }) };
  }
  ...
  if (!KEY || !SECRET) {
    console.error('[sync] Missing Schoology API credentials in environment');
    return {
      statusCode: 500,
      headers: { ...cors, 'Content-Type': 'application/json' },
      body: JSON.stringify({ ok: false, error: 'Server configuration error. Please try again later.' }),
    };
  }
  ...
  if (!accessToken || !accessSecret) {
    console.warn('[sync] Missing access tokens from client');
    return {
      statusCode: 401,
      headers: { ...cors, 'Content-Type': 'application/json' },
      body: JSON.stringify({ ok: false, needs_auth: true, error: 'Your session expired. Please sign in with Schoology again.' }),
    };
  }
  ...
  try {
    ...
    let friendlyMsg = 'Failed to sync your data. Check your internet and try again.';

    if (isAuthErr) {
      console.error('[sync] Authentication error:', err.message);
      friendlyMsg = 'Your Schoology connection expired. Please sign in again.';
    } else {
